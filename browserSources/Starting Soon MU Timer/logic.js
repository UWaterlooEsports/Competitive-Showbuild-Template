...
function updateElements(response) {
    ...
    //Matchup Text
    //------
    fadeTextChange('leftTeam',values[0][0],cBlack);
    fadeTextChange(`score`,`${values[0][4]} - ${values[1][4]}`,cBlack);
    fadeTextChange(`rightTeam`,values[1][0],cBlack);

    //debugging
    /*
    values.forEach(function(lol) {
        var output;
        lol.forEach(function(xd){
            output += `${xd} `;
        });
        console.log(output);
    });*/
...
async function fadeTextChange(elementID, newText, color) { //async so it doesn't delay the function that calls it
    //This is the overloaded fadeTextChange that can also take a new color
    let doc = document.getElementById(elementID);

    if (doc.textContent == newText) { //if element hasn't changed since last update, don't do anything
